...
class AdminController extends Controller
{
    ...
    public function index_addAdmin(Request $request)
    {
        ...
        $result = $this->datatable->handle(
            ...
            'employee_masterlist',
            [
                // SELECT * FROM tspi_hr_db.employee_masterlist where ACCSTATUS = 1 and DEPARTMENT IN ('MIS', 'Quality Management System') and Station IN ('MIS', 'Training & Development') and TEAM in (1,2);

                'conditions' => function ($query) use ($adminEmpIDs) {
                    return $query
                        ->where('ACCSTATUS', 1)
                        ->whereIn('STATION', ['MIS', 'Training & Development'])
                        ->whereIn('DEPARTMENT', ['Quality Management System', 'MIS'])
