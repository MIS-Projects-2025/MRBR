...
export default function ReservationHistory({ tableData, tableFilters, rooms }) {
    ...
    const openView = async (row) => {
        ...
        const res = await fetch(
            route("reservation.history.log", row.reservation_id),
        );
    ...
    const filteredHistoryLog = historyLog.filter(
        (log) =>
        ...
        <AuthenticatedLayout>
            ...
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-2xl font-bold text-teal-500">
                    <i className="fa-solid fa-book-open"></i> Reservation
                    History
                </h1>
            </div>
                ...
                {selectedRow && (
                    <div className="space-y-4">
                        ...
                        <div className="p-3 bg-teal-100 rounded-lg text-sm space-y-1">
                            ...
                            <p>
                                <b>Event:</b> {selectedRow.event_type}
                            </p>
                        </div>
                                ...
                                historyLog.map((log, index) => {
                                    ...
                                    return (
                                        <div key={index} className="relative">
                                            <div
                                                className={`absolute -left-[9px] top-1 w-3 h-3 rounded-full ${dotColor[log.status] || "bg-gray-400"}`}
                                            ></div>

                                            <div className="text-sm space-y-1">
                                                <span
                                                    className={`inline-flex px-2 py-1 text-xs rounded-full ${statusColor[log.status] || "bg-gray-100"}`}
                                                >
                                                    {log.status || "unknown"}
                                                </span>
